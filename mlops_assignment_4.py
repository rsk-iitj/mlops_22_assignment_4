...
split_frac = [.6,.65,.7,.75,.8]
dev_frac = [.1,.12,.13,.14,0.15]
...
for k in range(5):
    ...
    for i,clf in enumerate(model_of_choices):
        X_train, y_train, X_dev, y_dev, X_test, y_test = train_dev_test_split(
            data, label, split_frac[k],dev_frac[k]
        )
...
colours=[['r','g'],['b','k'],['c','m'],['y','r'],['g','b'],['r','g'],['b','k'],['c','m'],['y','r'],['g','b']]
df_cls = pd.DataFrame(z_l,columns=['SVC','DT'])
x_dif = [1,2,3,4,5]
...
for i in range(5):
    for j in range(2):
        lis_A = list(df_cls.SVC.apply(lambda x: x[count]))
        lis_B = list(df_cls.DT.apply(lambda x: x[count]))
        thisClr = colours[i]
        axes[i][j].plot(x_dif, lis_A,thisClr[0],label="SVC",marker='o')
        axes[i][j].plot(x_dif, lis_B,thisClr[1], label="Decision Tree",marker='o')
        axes[i][j].set_title(f"Trend for Labeled Class - {count}")
        #axes[i][j].set_xlabel()
        count+=1
        axes[i][j].legend()
        axes[i][j].set_xticks(x_dif)

for ax in axes.flat:
    ax.set(xlabel='Runs', ylabel='Accuracy')
...
for i in range(len(z_l)):
    ...
    axes[i].set_title(f"Trend for Run - {i+1}")
    axes[i].legend()

for ax in axes.flat:
    ax.set(xlabel='Class Labels', ylabel='Accuracy')
